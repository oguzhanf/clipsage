...
namespace ClipSage.Core.Logging
{
    public class Logger
    {
        ...
        public void Initialize(string cacheFolderPath)
        {
            ...
            try
            {
                ...
                {
                    FileName = Path.Combine(_logFolderPath, "clipsage-${shortdate}.log"),
                    Layout = "${longdate} [${level:uppercase=true}] [${machinename}] ${message} ${exception:format=tostring}",
                    ArchiveFileName = Path.Combine(_logFolderPath, "archive", "clipsage-{#}.log"),
